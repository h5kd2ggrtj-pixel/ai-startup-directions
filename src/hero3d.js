...
const legend=canvas?.parentElement?.insertBefore(Object.assign(document.createElement('div'),{className:'money-3d-legend',innerHTML:'<span><i></i>基础设施</span><span><i></i>平台与模型</span><span><i></i>应用与现金流</span><small>移动鼠标观察产业层级</small>'}),canvas.nextSibling);
...
if(canvas&&!reduce&&!constrained&&globalThis.WebGLRenderingContext){try{
 ...
 resize();canvas.hidden=false;canvas.classList.add('is-ready');legend?.classList.add('is-ready');fallback?.classList.add('is-3d-ready');raf=requestAnimationFrame(animate);
}catch(e){canvas.hidden=true;legend?.remove()}}
